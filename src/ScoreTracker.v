module ScoreTracker(PlayerID, isGuest, ScoreIn, ScoreRequest, PersonalBest, GlobalWinner, ScoreValid, clk, rst);
    ...
    reg [5:0] RAMAddress; // Address for ScoreRAM (6-bit for 64-word depth)
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            State <= RAM_INIT; // Initialize FSM state
        end else begin
            case (State)
                RAM_INIT: begin
                    RAMAddress <= {3'b000, PlayerID}; // Map 3-bit PlayerID to 6-bit RAM address
                ...
                COMPARE_SCORES: begin
                    if (ScoreIn > PersonalBest && !isGuest[0]) begin // If new score is better and not a guest
                ...
                UPDATE_GLOBAL: begin
                    if (!isGuest[0]) begin // Only update global winner if not a guest
